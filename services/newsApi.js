 ...
 * @param {Array} type A Array of params that you want to search to. The strings inside the array will be searched with a OR operator. Array Ex: ['technology', 'computing',], in URL will be: techonology OR computing.
...
export const getNews = async ({ trustSources = [], type = ['technology', 'computing', 'IT', 'computers', 'artificial intelligence', 'AI'], language = 'en', pageSize = 5, date = 'recent' }) => {

    //URL to get the news.

    const baseUrl = 'https://newsapi.org/v2/everything';
    const apiKey = process.env.NEWS_API; //API KEY from newsapi.org

    const params = new URLSearchParams();

    /* Insert the news type in the request */
    if (type.length > 0) {
        const query = type.map(keyword => (keyword.includes(' ') ? `"${keyword}"` : keyword)).join(' OR ');
    ...
    try {
        ...
        return console.log({ isError: true, message: "Error searching news" });
    }
};